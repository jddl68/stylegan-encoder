...
def create_variable_for_generator(name, batch_size):
    return tf.get_variable('learnable_dlatents',
                           shape=(batch_size, 14, 512), # Modified from 18 to 14
                           dtype='float32',
...
class Generator:
    def __init__(self, model, batch_size, randomize_noise=False):
        ...
        self.initial_dlatents = np.zeros((self.batch_size, 14, 512)) # Modified from 18 to 14
    ...
    def set_dlatents(self, dlatents):
        assert (dlatents.shape == (self.batch_size, 14, 512)) # Modified from 18 to 14
